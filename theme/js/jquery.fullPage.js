...
(function($) {
	$.fn.fullpage = function(options) {
		...
		options = $.extend({
			...
			'sectionSelector': '.section[data-fptooltip!="footer"]',
			'slideSelector': '.slide',
		...
		$.fn.fullpage.setAutoScrolling = function(value){
			...
			if(options.autoScrolling){
				$('html.home-slider, body.home-slider').css({
					'overflow' : 'hidden',
					'height' : '100%'
				});
				...
				$('html.home-slider, body.home-slider').scrollTop(element.position().top);
			}
		...
		*/
		...
		if (options.navigation) {
			$('body.home-slider').append('<div id="fp-nav"><ul></ul></div>');
			var nav = $('#fp-nav');
		...
		$('.fp-section').each(function(index){
			...
			if(options.fixedElements && options.css3){
				$(options.fixedElements).appendTo('body.home-slider');
			}
			...
			if(options.menu && options.css3 && $(options.menu).closest('.fullpage-wrapper').length){
				$(options.menu).appendTo('body.home-slider');
			}
			...
			if(destiny.length){
				...
				if(!options.animateAnchor && section.length){
					...
					if(options.autoScrolling){
						...
						//scrolling the page to the section with no animation
						$('html.home-slider, body.home-slider').scrollTop(section.position().top);
					}
		...
		*/
		...
		function scrollPage(element, callback, isMovementUp){
			...
			if(options.autoScrolling){
				...
				scrolledElement = 'html.home-slider, body.home-slider';
			}
		...
		*/
		...
		function resizeMe(displayHeight, displayWidth) {
			...
			if (displayHeight < 825 || displayWidth < 900) {
				...
				$("body.home-slider").css("font-size", newFontSize + '%');
			} else {
				$("body.home-slider").css("font-size", '100%');
			}
		}
		...
		function activateMenuElement(name){
			if(options.menu){
				$(options.menu).find('a.active').removeClass('active');
				$(options.menu).find('a[rel="'+name+'"]').addClass('active');
			}
		}
		...
		*/
		function destroyStructure(){
			...
			$('html.home-slider, body.home-slider').scrollTop(0);
